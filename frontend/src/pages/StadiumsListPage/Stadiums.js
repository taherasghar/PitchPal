...
function Stadiums() {
  ...
  const [searchTerm, setSearchTerm] = useState(stadium);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:4000/stadiums/results/?stadium=${searchTerm}&city=${city}`
      )
      .then(function (response) {
        setFetchedStadiums(response.data.stadiums);
